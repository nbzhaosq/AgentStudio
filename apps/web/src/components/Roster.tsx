...
export default function Roster({ agents, statuses, activities, candidates, onAddAgent, onRemoveAgent, sessions, onResetSession }: Props) {
  ...
  return (
    <aside className="w-56 shrink-0 overflow-y-auto border-l border-line bg-panel/40 p-4">
      <div className="mb-3 flex items-center justify-between">
        <h2 className="micro-label">成员 · {agents.length}</h2>
        {candidates.length > 0 && (
          <button
            className="rounded-md border border-line bg-hover px-1.5 py-0.5 font-mono text-[10px] text-text-3 transition-colors hover:border-line2 hover:text-text-1"
      ...
      {picking && (
        <div className="mb-3 overflow-hidden rounded-xl border border-line2 bg-panel2 shadow-xl shadow-black/40">
          {candidates.map((a) => (
            <button
              key={a.id}
              className="flex w-full items-center gap-2 px-2.5 py-2 text-left text-sm transition-colors hover:bg-active"
              onClick={() => {
                onAddAgent(a.id);
                setPicking(false);
              }}
            >
              ...
              <span className="truncate text-xs text-text-3">{a.name}</span>
            </button>
          ))}
        </div>
      )}
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2.5">
          <span className="relative flex h-8 w-8 items-center justify-center rounded-full bg-text-4 text-xs font-bold text-text-1">
            你
          </span>
          <span className="text-sm text-text-2">user</span>
        </div>
        {agents.map((a) => {
          ...
          return (
            <div key={a.id} className="group flex items-start gap-2.5">
              ...
              <div className="min-w-0 flex-1">
                <div className="font-mono text-xs text-text-2">@{a.id}</div>
              ...
              <button
                className="hidden shrink-0 rounded px-1 text-xs text-text-4 transition-colors hover:bg-active hover:text-red-400 group-hover:block"
                title="移出房间"
                onClick={() => {
        ...
        <div className="mt-5 border-t border-line pt-3">
          ...
          {sessions.map((s) => (
            <div key={s.agentId} className="group mb-1.5 flex items-center gap-1.5 text-[11px]">
              ...
              />
              <span className="truncate font-mono text-text-3" title={s.sessionId}>
                {s.sessionId.slice(0, 12)}…
              </span>
              <span className="shrink-0 text-text-4">{timeAgo(s.updatedAt)}</span>
              <button
                className="ml-auto hidden rounded px-1 text-text-3 transition-colors hover:bg-active hover:text-text-1 group-hover:block"
                title="重开该 agent 的会话"
